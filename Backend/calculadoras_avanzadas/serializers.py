...
class ScoreBishopSerializer(serializers.ModelSerializer):
    ...
    def get_interpretacion_detallada(self, obj):
        ...
        return {
            ...
            if obj.probabilidad_parto_espontaneo
            else 0,
            "componentes": {
    ...
    def validate_dilatacion_cervical(self, value):
        ...
        if value < 0 or value > 10:
            raise serializers.ValidationError(
                "La dilatación debe estar entre 0 y 10 cm",
            )
        return value

    def validate_borramiento_cervical(self, value):
        """Validate borramiento cervical"""
        if value < 0 or value > 100:
            raise serializers.ValidationError(
                "El borramiento debe estar entre 0 y 100%",
            )
        return value
...
class RiesgoPreeclampsiaSerializer(serializers.ModelSerializer):
    ...
    def validate_edad_gestacional_semanas(self, value):
        """Validate edad gestacional semanas"""
        if value < 11 or value > 14:
            raise serializers.ValidationError(
                "La evaluación óptima es entre 11-14 semanas",
            )
        return value

    def validate_edad_materna(self, value):
        """Validate edad materna"""
...
class CrecimientoFetalSerializer(serializers.ModelSerializer):
    ...
    def get_resumen_percentiles(self, obj):
        ...
        return {
            ...
            "biometria": {
                "dbp": {
                    "valor": float(obj.diametro_biparietal)
                    if obj.diametro_biparietal
                    else None,
                    "percentil": obj.percentil_dbp,
                },
                "cc": {
                    "valor": float(obj.circunferencia_cefalica)
                    if obj.circunferencia_cefalica
                    else None,
                    "percentil": obj.percentil_cc,
                },
                "ca": {
                    "valor": float(obj.circunferencia_abdominal)
                    if obj.circunferencia_abdominal
                    else None,
                    "percentil": obj.percentil_ca,
                },
                "lf": {
                    "valor": float(obj.longitud_femur) if obj.longitud_femur else None,
                    "percentil": obj.percentil_lf,
                },
            },
            "alertas": {
    ...
    def validate_edad_gestacional_semanas(self, value):
        ...
        if value < 16 or value > 42:
            raise serializers.ValidationError(
                "Edad gestacional fuera del rango válido para evaluación",
            )
        return value

    def validate_peso_fetal_estimado(self, value):
        """Validate peso fetal estimado"""

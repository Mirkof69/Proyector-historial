...
class ScoreBishop(models.Model):
    ...
    embarazo_id = models.IntegerField(null=True, blank=True, verbose_name='ID del Embarazo')
    medico_id = models.IntegerField(null=True, blank=True, verbose_name='ID del Médico')
    ...
    score_total = models.IntegerField(
        null=True,
        blank=True,
        verbose_name='Score Total de Bishop'
    )
    ...
    def calcular_score(self):
        ...
        if not all([self.dilatacion_cervical is not None, 
    ...
    def get_recomendacion_clinica(self):
        ...
        elif self.score_total <= 8:
            return "Inducción con precaución y monitoreo estrecho"
        else:
            return "Condiciones favorables para inducción del parto"
    
    def __str__(self):
        return f"Bishop Score {self.score_total or 'Pendiente'} - Paciente {self.paciente_id} - {self.fecha_evaluacion.date()}"
...
class RiesgoPreeclampsia(models.Model):
    ...
    diabetes_tipo1 = models.BooleanField(
        default=False,
        verbose_name='Diabetes Tipo 1'
    )
    
    diabetes_tipo2 = models.BooleanField(
        default=False,
        verbose_name='Diabetes Tipo 2'
    )
    
    lupus = models.BooleanField(
        default=False,
        verbose_name='Lupus Eritematoso Sistémico'
    )
    ...
    def calcular_riesgo(self):
        ...
        if riesgo_base > 0.5:  # 50%
            riesgo_base = 0.5
        
        self.riesgo_porcentaje = Decimal(str(round(riesgo_base * 100, 2)))
        self.riesgo_calculado = Decimal(str(round(1 / riesgo_base, 0))) if riesgo_base > 0 else Decimal('500')
    ...
    def __str__(self):
        return f"Riesgo Preeclampsia {self.riesgo_porcentaje or 'Pendiente'}% - Paciente {self.paciente_id}"
...
class CrecimientoFetal(models.Model):
    ...
    paciente_id = models.IntegerField(verbose_name='ID del Paciente')
    embarazo_id = models.IntegerField(null=True, blank=True, verbose_name='ID del Embarazo')
    ecografia_id = models.IntegerField(null=True, blank=True, verbose_name='ID de Ecografía')
    ...
    peso_fetal_estimado = models.IntegerField(
        null=True,
        blank=True,
        verbose_name='Peso Fetal Estimado (g)'
    )
    
    # Percentiles calculados
    percentil_dbp = models.IntegerField(null=True, blank=True, verbose_name='Percentil DBP')
    percentil_cc = models.IntegerField(null=True, blank=True, verbose_name='Percentil CC')
    percentil_ca = models.IntegerField(null=True, blank=True, verbose_name='Percentil CA')
    percentil_lf = models.IntegerField(null=True, blank=True, verbose_name='Percentil LF')
    percentil_peso = models.IntegerField(null=True, blank=True, verbose_name='Percentil Peso')
    
    # Evaluación del crecimiento
    clasificacion_peso = models.CharField(
        max_length=30,
        blank=True,
        verbose_name='Clasificación del Peso'
    )
    
    restriccion_crecimiento = models.BooleanField(
        default=False,
        verbose_name='¿Restricción de Crecimiento?'
    )
    
    macrosomia = models.BooleanField(
        default=False,
        verbose_name='¿Macrosomía?'
    )
    ...
    def calcular_percentiles(self):
        ...
        edad_decimal = self.edad_gestacional_semanas + ((self.edad_gestacional_dias or 0) / 7)
        ...
        if self.peso_fetal_estimado:
            ...
            if edad_decimal >= 20:
                peso_esperado = 50 * (edad_decimal - 20) ** 2 + 300  # Fórmula aproximada
...
class RiesgoCromosomico(models.Model):
    ...
    recomendacion = models.TextField(
        blank=True,
        verbose_name='Recomendación Clínica'
    )
    ...
    def calcular_riesgo_por_edad(self):
        ...
        riesgos_edad = {
            15: 1/1667, 16: 1/1667, 17: 1/1667, 18: 1/1667, 19: 1/1667,
            20: 1/1667, 21: 1/1667, 22: 1/1667, 23: 1/1667, 24: 1/1538,
            25: 1/1351, 26: 1/1176, 27: 1/1053, 28: 1/952, 29: 1/870,
            30: 1/800, 31: 1/741, 32: 1/690, 33: 1/645, 34: 1/606,
            35: 1/571, 36: 1/541, 37: 1/515, 38: 1/490, 39: 1/469,
            40: 1/449, 41: 1/431, 42: 1/414, 43: 1/398, 44: 1/383,
            45: 1/369, 46: 1/356, 47: 1/343, 48: 1/331, 49: 1/320, 50: 1/309
        }
    ...
    def get_interpretacion_completa(self):
        ...
        return {
            ...
            'clasificacion': self.get_clasificacion_down_display() if self.clasificacion_down else "No clasificado",
            'translucencia_nucal': f"{self.translucencia_nucal} mm" if self.translucencia_nucal else "No medida",
            'marcadores_bioquimicos': {
                'papp_a': f"{self.papp_a_mom} MoM" if self.papp_a_mom else "No disponible",
                'beta_hcg': f"{self.beta_hcg_mom} MoM" if self.beta_hcg_mom else "No disponible",
            },
            'recomendacion': self.recomendacion or "Complete los datos para obtener recomendaciones"
        }
    
    def __str__(self):
        return f"Screening Cromosómico - Paciente {self.paciente_id} - Riesgo: {self.riesgo_down_ajustado or 'Pendiente'}"
...
class DosisMedicamentos(models.Model):
    ...
    intervalo_dosis = models.CharField(
        max_length=50,
        blank=True,
        verbose_name='Intervalo entre Dosis'
    )
    ...
    precauciones = models.TextField(
        blank=True,
        verbose_name='Precauciones Especiales'
    )
    ...
    monitoreo_requerido = models.TextField(
        blank=True,
        verbose_name='Monitoreo Requerido'
    )
    ...
    def calcular_dosis_oxitocina(self):
        ...
        if self.indicacion == 'induccion_parto':
            ...
            self.monitoreo_requerido = "Monitoreo fetal continuo, contracciones uterinas, presión arterial"
        ...
        self.contraindicaciones = "Presentación anormal, placenta previa, cicatriz uterina previa"
    
    def calcular_dosis_sulfato_magnesio(self):
        """Cálculo de dosis de sulfato de magnesio"""
        peso = float(self.peso_materno)
        ...
        if self.indicacion in ['preeclampsia_severa', 'eclampsia']:
            ...
            self.via_administracion = "Intravenosa"
            self.duracion_tratamiento = "24 horas postparto o hasta 24h sin convulsiones"
            ...
            self.contraindicaciones = "Miastenia gravis, bloqueo cardíaco, insuficiencia renal severa"
    ...
    def calcular_dosis_nifedipino(self):
        ...
        if self.indicacion == 'tocolis_aguda':
            ...
            self.monitoreo_requerido = "Presión arterial, frecuencia cardíaca materna, actividad uterina"
            ...
            self.monitoreo_requerido = "Presión arterial cada 15 min x 1 hora, luego cada hora"
        
        self.precauciones = "Hipotensión materna, taquicardia, cefalea"
        self.contraindicaciones = "Hipotensión, insuficiencia cardíaca, estenosis aórtica"
    ...
    def calcular_dosis_labetalol(self):
        ...
        if self.indicacion == 'hipertension_gestacional':
            ...
            self.monitoreo_requerido = "Presión arterial, frecuencia cardíaca, bienestar fetal"
            self.precauciones = "Bradicardia materna, hipotensión, asma"
            self.contraindicaciones = "Asma severa, bloqueo cardíaco, insuficiencia cardíaca descompensada"
...
class HemorragiaObstetrica(models.Model):
    ...
    medidas_implementadas = models.TextField(
        blank=True,
        verbose_name='Medidas Implementadas'
    )
    
    # Transfusión
    requiere_transfusion = models.BooleanField(
        default=False,
        verbose_name='¿Requiere Transfusión?'
    )
    
    unidades_globulos_rojos = models.IntegerField(
        default=0,
        verbose_name='Unidades de Glóbulos Rojos'
    )
    
    unidades_plasma_fresco = models.IntegerField(
        default=0,
        verbose_name='Unidades de Plasma Fresco'
    )
    
    unidades_plaquetas = models.IntegerField(
        default=0,
        verbose_name='Unidades de Plaquetas'
    )
    
    # Resultado
    hemorragia_controlada = models.BooleanField(
        default=False,
        verbose_name='¿Hemorragia Controlada?'
    )
    
    requirio_cirugia = models.BooleanField(
        default=False,
        verbose_name='¿Requirió Cirugía?'
    )
    ...
    def calcular_indices(self):
        ...
        # Índice de shock
        if self.frecuencia_cardiaca and self.presion_arterial_sistolica and self.presion_arterial_sistolica > 0:
            self.indice_shock = Decimal(str(self.frecuencia_cardiaca / self.presion_arterial_sistolica))
    ...
    def calcular_necesidades_transfusion(self):
        ...
        if self.severidad_hemorragia in ["Severa", "Masiva"]:
            ...
            if self.perdida_sanguinea_estimada and self.perdida_sanguinea_estimada >= 1500:
                self.unidades_globulos_rojos = 4
                self.unidades_plasma_fresco = 4
                self.unidades_plaquetas = 1
            elif self.perdida_sanguinea_estimada and self.perdida_sanguinea_estimada >= 1000:
        ...
        if self.hemoglobina_inicial and float(self.hemoglobina_inicial) < 7.0:
            self.requiere_transfusion = True
            if self.unidades_globulos_rojos < 2:
                self.unidades_globulos_rojos = 2
...
class SufrimientoFetal(models.Model):
    ...
    aceleraciones_presentes = models.BooleanField(
        default=True,
        verbose_name='¿Aceleraciones presentes?'
    )
    
    # Desaceleraciones
    desaceleraciones_tardias = models.BooleanField(
        default=False,
        verbose_name='Desaceleraciones Tardías'
    )
    
    desaceleraciones_variables = models.BooleanField(
        default=False,
        verbose_name='Desaceleraciones Variables'
    )
    
    desaceleraciones_prolongadas = models.BooleanField(
        default=False,
        verbose_name='Desaceleraciones Prolongadas'
    )
    ...
    conducta_recomendada = models.TextField(
        blank=True,
        verbose_name='Conducta Recomendada'
    )
    
    requiere_intervencion_inmediata = models.BooleanField(
        default=False,
        verbose_name='¿Requiere Intervención Inmediata?'
    )
    ...
    def calcular_scores(self):
        ...
        if self.tipo_monitoreo in ['ctg_externo', 'ctg_interno']:
            ...
            # Desaceleraciones
            if not any([self.desaceleraciones_tardias, self.desaceleraciones_variables, self.desaceleraciones_prolongadas]):
                score += 2
    ...
    def determinar_clasificacion(self):
        ...
        if self.tipo_monitoreo in ['ctg_externo', 'ctg_interno']:
            # Clasificación NICHD
            if (self.fcf_basal and 110 <= self.fcf_basal <= 160 and
                self.variabilidad_fcf == 'moderada' and
                self.aceleraciones_presentes and
                not any([self.desaceleraciones_tardias, self.desaceleraciones_variables, self.desaceleraciones_prolongadas])):
                self.clasificacion_ctg = 'categoria_1'
                self.riesgo_sufrimiento_fetal = 'bajo'
            
            elif (self.desaceleraciones_tardias or 
                  self.desaceleraciones_prolongadas or
                  self.variabilidad_fcf == 'ausente' or
                  (self.fcf_basal and (self.fcf_basal < 110 or self.fcf_basal > 180))):
    ...
    def generar_recomendaciones(self):
        ...
        if self.clasificacion_ctg == 'categoria_1':
            ...
            recomendaciones.append("Evaluación cada 30 minutos en trabajo de parto activo")
            ...
            recomendaciones.append("Considerar estimulación fetal o cambios de posición")
        ...
        if self.indice_pulsatilidad_umbilical and float(self.indice_pulsatilidad_umbilical) > 1.45:
        ...
        if not recomendaciones:
            recomendaciones.append("Complete los datos para obtener recomendaciones específicas")
    ...
    def get_interpretacion_completa(self):
        ...
        return {
            ...
            'variabilidad': self.get_variabilidad_fcf_display() if self.variabilidad_fcf else "No evaluada",
            'clasificacion_ctg': self.get_clasificacion_ctg_display() if self.clasificacion_ctg else "No clasificado",
            'riesgo': self.get_riesgo_sufrimiento_fetal_display() if self.riesgo_sufrimiento_fetal else "No evaluado",
            'score_fisher': self.score_fisher if self.score_fisher is not None else "No calculado",
            'pbf_score': self.perfil_biofsico_score if self.perfil_biofsico_score is not None else "No calculado",
            'intervencion_inmediata': self.requiere_intervencion_inmediata,
            'recomendaciones': self.conducta_recomendada or "Complete los datos para obtener recomendaciones"
        }

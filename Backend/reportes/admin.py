...
class OpcionesDinamicas:
    ...
    def obtener_tipos_reporte():
        ...
        if opciones is None:
            try:
                opciones = list(
                    TipoReporte.objects.filter(activo=True).values_list("id", "nombre"),
                )
    ...
    def obtener_pacientes_activos():
        ...
        if opciones is None:
            try:
                # ✅ CIRCULAR IMPORT FIX: Usar apps.get_model() en lugar de import directo
                ...
                for p in pacientes:
                    ...
                    nombre_completo = (
                        f"{p.nombre} {apellidos} ({p.id_clinico or f'ID: {p.id}'})"
                    )
    ...
    def obtener_usuarios_medicos():
        ...
        if opciones is None:
            try:
                # ✅ CIRCULAR IMPORT FIX: Usar get_user_model() en lugar de import directo
    ...
    def obtener_embarazos_activos():
        ...
        if opciones is None:
            try:
                # ✅ CIRCULAR IMPORT FIX: Usar apps.get_model()
                ...
                for e in embarazos:
                    try:
                        if hasattr(e, "paciente") and e.paciente:
                            ...
                            descripcion = (
                                f"Embarazo ID: {e.id} - Gesta: {e.numero_gesta}"
                            )
    ...
    def obtener_consultas_predefinidas():
        ...
        return [
            ...
            (
                "promedio_controles",
                "Promedio de Controles por Embarazo",
            ...
            (
                "calculadoras_uso_mes",
                "Uso de Calculadoras Este Mes",
...
class TipoReporteAdminForm(forms.ModelForm):
    ...
    def __init__(self, *args, **kwargs):
        ...
        for _codigo, nombre, consulta in consultas:
...
class DashboardKPIAdminForm(forms.ModelForm):
    ...
    class Meta:
        ...
        widgets = {
            ...
            "valor_anterior": forms.NumberInput(
                attrs={"step": "0.01", "class": "form-control"},
            ),
            "meta_minima": forms.NumberInput(
                attrs={"step": "0.01", "class": "form-control"},
            ),
            "meta_optima": forms.NumberInput(
                attrs={"step": "0.01", "class": "form-control"},
            ),
            "orden_dashboard": forms.NumberInput(
...
class AlertaMedicaAdminForm(forms.ModelForm):
    ...
    class Meta:
        ...
        widgets = {
            ...
            "accion_recomendada": forms.Textarea(
                attrs={"rows": 3, "class": "form-control"},
            ),
            "protocolo_seguimiento": forms.Textarea(
                attrs={"rows": 3, "class": "form-control"},
            ),
            "comentario_revision": forms.Textarea(
                attrs={"rows": 3, "class": "form-control"},
            ),
            "comentario_resolucion": forms.Textarea(
                attrs={"rows": 3, "class": "form-control"},
            ),
            ...
            "valor_umbral": forms.NumberInput(
                attrs={"step": "0.01", "class": "form-control"},
            ),
            "tiempo_escalamiento_horas": forms.NumberInput(
                attrs={"min": "1", "class": "form-control"},
            ),
        }
...
class TipoReporteAdmin(admin.ModelAdmin):
    ...
    fieldsets = (
        ...
        (
            ...
            {"fields": (("get_resumen_configuracion",),), "classes": ("collapse",)},
        ),
    )
    ...
    def get_formatos_count(self, obj):
        """Get formatos count"""
        count = sum(
            [obj.formato_pdf, obj.formato_excel, obj.formato_csv, obj.formato_json],
        )
        return format_html(
    ...
    def get_resumen_configuracion(self, obj):
        ...
        # Filtros disponibles
        filtros = []
        if obj.incluir_fecha_inicio:
            filtros.append(" Fecha inicio")
        if obj.incluir_fecha_fin:
            filtros.append(" Fecha fin")
        if obj.incluir_paciente:
            filtros.append(" Paciente")
        if obj.incluir_medico:
            filtros.append("‍⚕️ Médico")
        if obj.incluir_servicio:
            filtros.append(" Servicio")
        ...
        roles = []
        if obj.permitir_medico:
            roles.append("Médicos")
        if obj.permitir_enfermero:
            roles.append("Enfermeros")
        if obj.permitir_administrador:
            roles.append("Administradores")
        if obj.permitir_director:
        ...
        config = []
        if obj.automatico:
            config.append(" Generación automática")
        if obj.confidencial:
            config.append(" Requiere autorización")
        if obj.requiere_parametros:
            config.append("⚙️ Requiere parámetros")
...
class ReporteGeneradoAdmin(admin.ModelAdmin):
    ...
    fieldsets = (
        ...
        (
            " Estadísticas de Uso",
            {"fields": (("get_estadisticas_uso",),), "classes": ("collapse",)},
        ),
        (
    ...
    def get_progreso_display(self, obj):
        ...
        if obj.estado == "completado":
            return format_html(
                '<span style="color: #28a745; font-weight: bold; font-size: 12px;">✅ Completado</span>',
            )
        if obj.estado == "procesando":
            return format_html(
                '<span style="color: #007bff; font-weight: bold; font-size: 12px;">⚙️ Procesando</span>',
            )
        if obj.estado == "error":
            return format_html(
                '<span style="color: #dc3545; font-weight: bold; font-size: 12px;">❌ Error</span>',
            )
        return format_html(
    ...
    def get_tamano_display(self, obj):
        ...
        if obj.tamano_archivo:
            try:
                tamano_bytes = int(obj.tamano_archivo)
    ...
    def esta_expirado_badge(self, obj):
        ...
        if obj.esta_expirado():
            return format_html(
                '<span style="color: #dc3545; font-size: 12px; font-weight: bold;">⚠️ Expirado</span>',
            )
        return format_html(
    ...
    def get_progreso_detallado(self, obj):
        ...
        if obj.tamano_archivo:
            info.append(f" Tamaño: {self.get_tamano_display(obj)}")

        if obj.accesos_descarga > 0:
            info.append(
                f" {obj.accesos_descarga} descarga{'s' if obj.accesos_descarga != 1 else ''}",
            )

        if obj.fecha_completado:
            info.append(
                f" Completado: {obj.fecha_completado.strftime('%d/%m/%Y %H:%M')}",
            )
    ...
    def get_estadisticas_uso(self, obj):
        ...
        if obj.accesos_descarga > 0 and obj.fecha_completado:
            ...
            if dias_desde_completado > 0:
                ...
                estadisticas.append(
                    f" Frecuencia de descarga: {frecuencia:.2f} por día",
                )

        # Eficiencia de procesamiento
        if obj.total_registros and obj.tiempo_procesamiento:
            try:
                registros_por_segundo = obj.total_registros / int(
                    obj.tiempo_procesamiento,
                )
                estadisticas.append(
                    f"⚡ Velocidad: {registros_por_segundo:.2f} registros/segundo",
                )
...
class DashboardKPIAdmin(admin.ModelAdmin):
    ...
    fieldsets = (
        ...
        (" Configuración Visual", {"fields": (("icono",),)}),
        (
    ...
    def tendencia_badge(self, obj):
        ...
        if tendencia == "subiendo":
            return format_html(
                '<span style="color: #28a745; font-size: 14px; font-weight: bold;"> ↗️</span>',
            )
        if tendencia == "bajando":
            return format_html(
                '<span style="color: #dc3545; font-size: 14px; font-weight: bold;"> ↘️</span>',
            )
        if tendencia == "estable":
            return format_html(
                '<span style="color: #ffc107; font-size: 14px; font-weight: bold;">➡️ =</span>',
            )
        return format_html(
    ...
    def estado_vs_meta_badge(self, obj):
        ...
        if estado == "optimo":
            return format_html(
                '<span style="color: #28a745; font-size: 12px; font-weight: bold;"> Óptimo</span>',
            )
        if estado == "bajo_minimo":
            return format_html(
                '<span style="color: #dc3545; font-size: 12px; font-weight: bold;">⚠️ Bajo</span>',
            )
        if estado == "en_progreso":
            return format_html(
                '<span style="color: #ffc107; font-size: 12px; font-weight: bold;"> Progreso</span>',
            )
        return format_html(
    ...
    def get_rendimiento_kpi(self, obj):
        ...
        if obj.valor_actual and obj.meta_optima:
            try:
                porcentaje_meta = (
                    float(obj.valor_actual) / float(obj.meta_optima)
                ) * 100
        ...
        if obj.valor_actual and obj.meta_minima:
            try:
                porcentaje_minimo = (
                    float(obj.valor_actual) / float(obj.meta_minima)
                ) * 100
        ...
        if obj.valor_actual and obj.valor_anterior:
            try:
                ...
                info.append(
                    f" Variación: {signo}{variacion:.2f} ({signo}{porcentaje_variacion:.1f}%)",
                )
            except (ValueError, TypeError, ZeroDivisionError):
                pass
        ...
        visibilidad = []
        if obj.visible_medico:
            visibilidad.append("Médicos")
        if obj.visible_administrador:
            visibilidad.append("Administradores")
        if obj.visible_director:
            visibilidad.append("Director")
        ...
        if obj.valor_actual and obj.valor_anterior:
            try:
                ...
                if actual > anterior:
                    ...
                    analisis.append(
                        f" Incremento de {diferencia:.2f} ({porcentaje:.1f}%)",
                    )
                elif actual < anterior:
                    diferencia = anterior - actual
                    porcentaje = (diferencia / anterior) * 100
                    analisis.append(
                        f" Disminución de {diferencia:.2f} ({porcentaje:.1f}%)",
                    )
                else:
        ...
        if obj.valor_actual and obj.meta_optima:
            try:
                ...
                if cumplimiento < 50:
                    ...
                    analisis.append(
                        " Recomendación: Buen progreso, continuar esfuerzos",
                    )
            except (ValueError, TypeError, ZeroDivisionError):
                pass
...
class AlertaMedicaAdmin(admin.ModelAdmin):
    ...
    fieldsets = (
        ...
        (
            ...
            {
                "fields": (
                    ("usuario_revision", "usuario_resolucion"),
                    ("comentario_revision",),
        ...
        (
            ...
            {"fields": (("get_historial_cambios",),), "classes": ("collapse",)},
        ),
        (
            " Notificaciones",
            {
                "fields": (("notificacion_enviada", "recordatorio_enviado"),),
                "classes": ("collapse",),
            },
        ),
    )
    ...
    def get_tiempo_vida_alerta(self, obj):
        ...
        if obj.escalamiento_automatico and obj.tiempo_escalamiento_horas:
            tiempo_escalamiento = obj.fecha_creacion + timedelta(
                hours=obj.tiempo_escalamiento_horas,
            )
    ...
    def get_historial_cambios(self, obj):
        ...
        historial.append(
            f" Creada: {obj.fecha_creacion.strftime('%d/%m/%Y %H:%M') if obj.fecha_creacion else 'No disponible'}",
        )

        if obj.fecha_revision:
            historial.append(
                f"️ Revisada: {obj.fecha_revision.strftime('%d/%m/%Y %H:%M')}",
            )
            if obj.usuario_revision:
                historial.append(f"    Por usuario: {obj.usuario_revision}")

        if obj.fecha_resolucion:
            historial.append(
                f"✅ Resuelta: {obj.fecha_resolucion.strftime('%d/%m/%Y %H:%M')}",
            )
            if obj.usuario_resolucion:
                historial.append(f"    Por usuario: {obj.usuario_resolucion}")
...
class AuditoriaReporteAdmin(admin.ModelAdmin):
    ...
    def get_queryset(self, request):
        ...
        return (
            super()
            .get_queryset(request)
            .select_related("reporte_generado__tipo_reporte", "usuario")
            .only(
                "id",
                "accion",
                "usuario_id",
    ...
    def get_ubicacion_acceso(self, obj):
        ...
        return format_html(
            ...
            obj.fecha_accion.strftime("%d/%m/%Y %H:%M:%S")
            if obj.fecha_accion
            else "Sin fecha",
        )
    ...
    def get_analisis_cumplimiento(self, obj):
        ...
        if obj.justificacion_acceso:
            analisis.append(
                f" Justificación proporcionada: {len(obj.justificacion_acceso)} caracteres",
            )
        ...
        if obj.accion == "descarga":
            ...
            analisis.append(
                "✏️ Modificación de datos - Requiere justificación detallada",
            )
        ...
        if obj.fecha_accion:
            ...
            if hora < 6 or hora > 22:
                analisis.append(
                    " Acceso fuera de horario laboral - Revisar justificación",
                )

            # Verificar si es fin de semana
            if obj.fecha_accion.weekday() >= 5:  # Sábado=5, Domingo=6
                analisis.append(
                    " Acceso en fin de semana - Verificar emergencia médica",
                )

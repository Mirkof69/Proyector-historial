...
if errorlevel 1 (
    ...
    if not errorlevel 1 (
        ...
        echo   [AVISO] Redis no encontrado. Celery funcionara en modo degradado.
    )
) else (
...
start "Django Backend :8000" cmd /k "cd /d "%ROOT%Backend" && python manage.py runserver 127.0.0.1:8000"
timeout /t 3 /nobreak >nul
echo   [OK] Django: http://127.0.0.1:8000

:: 4. Microservicio IA FastAPI
...
start "Celery Worker" cmd /k "cd /d "%ROOT%Backend" && set "DJANGO_SETTINGS_MODULE=settings" && python -m celery -A celery_app worker --loglevel=info -P solo"
timeout /t 2 /nobreak >nul
...
echo   Django Backend:     http://127.0.0.1:8000/api/
echo   Django Admin:       http://127.0.0.1:8000/admin/
echo   Microservicio IA:   http://127.0.0.1:8001/docs
echo ----------------------------------------------------------------
echo   Admin: MirkofAdmin@fetalmedical.com  /  25693
echo ================================================================
echo.

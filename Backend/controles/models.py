...
# Choices para campos selectivos - DEFINIDOS AFUERA PARA EVITAR ERRORES DE SCOPE
...
class ControlPrenatal(models.Model):
    ...
    talla = models.DecimalField(
        ...
        decimal_places=2,
        validators=[
            MinValueValidator(Decimal("130.0")),
            MaxValueValidator(Decimal("200.0")),
        ],
    ...
    def presion_arterial_media(self):
        ...
        if self.presion_arterial_sistolica and self.presion_arterial_diastolica:
            pam = (
                self.presion_arterial_sistolica + (2 * self.presion_arterial_diastolica)
            ) / 3
            return round(pam, 2)
        return None
    ...
    def tiene_hipertension(self):
        ...
        if self.presion_arterial_sistolica and self.presion_arterial_diastolica:
            return (
                self.presion_arterial_sistolica >= 140
                or self.presion_arterial_diastolica >= 90
            )
        return False
    ...
    def tiene_prehipertension(self):
        ...
        if self.presion_arterial_sistolica and self.presion_arterial_diastolica:
            return (
                self.presion_arterial_sistolica >= 120
                or self.presion_arterial_diastolica >= 80
            )
        return False
    ...
    def fcf_es_anormal(self):
        ...
        if self.frecuencia_cardiaca_fetal:
            return (
                self.frecuencia_cardiaca_fetal < 110
                or self.frecuencia_cardiaca_fetal > 160
            )
        return False
    ...
    def riesgo_preeclampsia(self):
        ...
        return {"nivel": nivel, "puntuacion": riesgo, "factores": factores}
    ...
    def crecimiento_fetal(self):
        ...
        resultado = {
            "altura_uterina": float(self.altura_uterina)
    ...
    def clean(self):
        """✅ VALIDACIONES CRÍTICAS DEL CONTROL PRENATAL"""
        super().clean()
        errors = {}
        # Fecha de control no debe ser futura
        ...
        if self.presion_arterial_sistolica and self.presion_arterial_diastolica:
            ...
            # Validar rangos mortales
            if self.presion_arterial_sistolica > 200:
                errors["presion_arterial_sistolica"] = (
                    f"Presión sistólica de {self.presion_arterial_sistolica} mmHg es peligrosamente alta. Máximo 200 mmHg."
                )

            if self.presion_arterial_diastolica > 120:
                errors["presion_arterial_diastolica"] = (
                    f"Presión diastólica de {self.presion_arterial_diastolica} mmHg es peligrosamente alta. Máximo 120 mmHg."
                )

            # Validar valores muy bajos (shock)
            if self.presion_arterial_sistolica < 80:
                errors["presion_arterial_sistolica"] = (
                    f"Presión sistólica de {self.presion_arterial_sistolica} mmHg es peligrosamente baja. Mínimo 80 mmHg."
                )

        # ✅ 2. VALIDAR PESO
        if self.peso_actual:
            if self.peso_actual > 180:
                errors["peso_actual"] = (
                    f"Peso de {self.peso_actual} kg parece muy alto. Revisar medición. Máximo recomendado 180 kg."
                )
            elif self.peso_actual < 35:
                errors["peso_actual"] = (
                    f"Peso de {self.peso_actual} kg parece muy bajo. Revisar medición. Mínimo recomendado 35 kg."
                )

        # ✅ 3. VALIDAR ALTURA UTERINA
        if self.altura_uterina:
            if self.altura_uterina > 45:
                errors["altura_uterina"] = (
                    f"Altura uterina de {self.altura_uterina} cm es anormalmente alta. Máximo 45 cm. Verificar embarazo gemelar o polihidramnios."
                )

            # Validar coherencia con semanas de gestación
            if self.semanas_gestacion >= 20:
                if self.altura_uterina < (self.semanas_gestacion - 4):
                    errors["altura_uterina"] = (
                        f"Altura uterina de {self.altura_uterina} cm muy baja para {self.semanas_gestacion} semanas. Esperado: {self.semanas_gestacion - 2} a {self.semanas_gestacion + 2} cm."
                    )
                elif self.altura_uterina > (self.semanas_gestacion + 4):
                    errors["altura_uterina"] = (
                        f"Altura uterina de {self.altura_uterina} cm muy alta para {self.semanas_gestacion} semanas. Esperado: {self.semanas_gestacion - 2} a {self.semanas_gestacion + 2} cm."
                    )

        # ✅ 4. VALIDAR FRECUENCIA CARDÍACA FETAL
        if self.frecuencia_cardiaca_fetal:
            if self.frecuencia_cardiaca_fetal < 100:
                errors["frecuencia_cardiaca_fetal"] = (
                    f"FCF de {self.frecuencia_cardiaca_fetal} lpm es peligrosamente baja. BRADICARDIA SEVERA. Rango normal: 110-160 lpm."
                )
            elif self.frecuencia_cardiaca_fetal > 180:
                errors["frecuencia_cardiaca_fetal"] = (
                    f"FCF de {self.frecuencia_cardiaca_fetal} lpm es peligrosamente alta. TAQUICARDIA SEVERA. Rango normal: 110-160 lpm."
                )

        # ✅ 5. VALIDAR TEMPERATURA
        if self.temperatura:
            if float(self.temperatura) > 40:
                errors["temperatura"] = (
                    f"Temperatura de {self.temperatura}°C es peligrosamente alta. Máximo 40°C."
                )
            elif float(self.temperatura) < 35:
                errors["temperatura"] = (
                    f"Temperatura de {self.temperatura}°C es peligrosamente baja (hipotermia). Mínimo 35°C."
                )

        # ✅ 6. VALIDAR SEMANAS DE GESTACIÓN
        if self.semanas_gestacion > 42:
            errors["semanas_gestacion"] = (
                f"{self.semanas_gestacion} semanas excede el límite normal (42 semanas). Embarazo prolongado."
            )

...
class OpcionesDinamicas:
    ...
    def obtener_consultas_predefinidas():
        ...
        consultas = [
            ('pacientes_total', 'Total de Pacientes', 'SELECT COUNT(*) FROM pacientes_paciente WHERE activo = true'),
            ('embarazos_activos', 'Embarazos Activos', 'SELECT COUNT(*) FROM embarazos_embarazo WHERE activo = true'),
            ('controles_mes', 'Controles Este Mes', "SELECT COUNT(*) FROM controles_controlprenatal WHERE fecha_control >= DATE_TRUNC('month', CURRENT_DATE)"),
            ('ecografias_mes', 'Ecografías Este Mes', "SELECT COUNT(*) FROM ecografias_ecografia WHERE fecha_ecografia >= DATE_TRUNC('month', CURRENT_DATE)"),
            ('partos_mes', 'Partos Este Mes', "SELECT COUNT(*) FROM partos_parto WHERE fecha_parto >= DATE_TRUNC('month', CURRENT_DATE)"),
            ('citas_pendientes', 'Citas Pendientes', "SELECT COUNT(*) FROM citas_cita WHERE estado = 'programada'"),
        ]
        return consultas

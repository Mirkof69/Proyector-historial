...
class Ecografia(models.Model):
    ...
    tipo_ecografia = models.CharField(
        max_length=30, choices=TIPO_CHOICES, verbose_name="Tipo de Ecografía",
    )
    ...
    indice_liquido_amniotico = models.DecimalField(
        max_digits=4, decimal_places=1, null=True, blank=True, verbose_name="ILA (cm)",
    )
    ...
    localizacion_placenta = models.CharField(
        max_length=100, blank=True, verbose_name="Localización Placenta",
    )
    ...
    proxima_ecografia_recomendada = models.DateField(
        null=True, blank=True, verbose_name="Próxima Ecografía Recomendada",
    )
    ...
    def get_percentil_ila(self):
        ...
        semana_ref = min(
            referencias_ila.keys(), key=lambda x: abs(x - self.edad_gestacional_semanas),
        )
...
class BiometriaFetal(models.Model):
    ...
    diametro_abdominal_transverso = models.DecimalField(
        max_digits=4, decimal_places=1, null=True, blank=True, verbose_name="DAT (mm)",
    )
    ...
    peso_fetal_estimado = models.IntegerField(
        null=True, blank=True, verbose_name="Peso Fetal Est. (g)",
    )
    ...
    def calcular_peso_fetal_hadlock(self):
        """Calcula peso fetal usando fórmula de Hadlock 1985 (4 parámetros).
        Referencia: Hadlock FP et al. Radiology 1985;150:535-540.
        log10(EFW) = 1.3596 - 0.00386*AC*FL + 0.0064*HC + 0.00061*BPD*AC + 0.0425*AC + 0.174*FL
        Todos los valores deben estar en mm. El resultado es en gramos.
        Error reportado: ±15% (comparable a otras fórmulas biométricas).
        """
        if not all(
            [
                self.diametro_biparietal,
                self.circunferencia_cefalica,
                self.circunferencia_abdominal,
                self.longitud_femur,
            ],
        ):
            return None
...
class AnatomiaFetal(models.Model):
    ...
    # Cara
    perfil_facial_normal = models.BooleanField(
    ...
    pulmones_normales = models.BooleanField(
        default=True, verbose_name="Pulmones Normales",
    )

    # Abdomen
    estomago_normal = models.BooleanField(default=True, verbose_name="Estómago Normal")
    rinones_normales = models.BooleanField(
        default=True, verbose_name="Riñones Normales",
    )
    ...
    # Extremidades
    extremidades_superiores_normales = models.BooleanField(
        default=True, verbose_name="Extremidades Superiores Normales",
    )
    extremidades_inferiores_normales = models.BooleanField(
        default=True, verbose_name="Extremidades Inferiores Normales",
    )

    # Genitales
    genitales_visibles = models.BooleanField(
        default=False, verbose_name="Genitales Visibles",
    )
    ...
    hallazgos_anormales = models.TextField(
        blank=True, verbose_name="Hallazgos Anormales",
    )
    ...
    hueso_nasal_presente = models.BooleanField(
        null=True, blank=True, verbose_name="Hueso Nasal Presente",
    )
    ...
    def get_evaluacion_anatomica(self):
        ...
        campos_normales = sum(
            1 for campo in campos_anatomicos if getattr(self, campo, True)
        )
...
class AnexosFetales(models.Model):
    ...
    placenta_insercion_cordon = models.CharField(
        max_length=100, blank=True, verbose_name="Inserción del Cordón",
    )

    placenta_previa = models.BooleanField(default=False, verbose_name="Placenta Previa")
    ...
    circular_cordon = models.BooleanField(
        default=False, verbose_name="Circular de Cordón",
    )
    ...
    polihidramnios = models.BooleanField(default=False, verbose_name="Polihidramnios")
    oligohidramnios = models.BooleanField(default=False, verbose_name="Oligohidramnios")
...
class ImagenEcografia(models.Model):
    ...
    orden = models.PositiveIntegerField(
        default=1, verbose_name="Orden", help_text="Orden de visualización (1, 2, 3...)",
    )
    ...
    def analizar_con_ia(self):
        """Analiza la imagen con CNN real via cnn_service (Circuit Breaker).
        Intenta microservicio FastAPI primero; cae a análisis Pillow local si no disponible.
        Requiere que la imagen esté guardada en disco (self.imagen debe tener path).
        """
        if not self.imagen:
            return {"error": "No hay imagen para analizar", "procesado": False}

        try:
            from ia_medica.services.cnn_service import cnn_service

            resultado = cnn_service.analizar_imagen(
                self.imagen.path, modelo="efficientnet",
            )
            ...
            analisis = {
                ...
                "hallazgos": resultado.get("anomalias_detectadas", []),
                "mediciones_detectadas": resultado.get("medidas_estimadas") or {},
                "anomalias_detectadas": resultado.get("anomalias_detectadas", []),
                "alertas": resultado.get("alertas", []),
                "calidad_tecnica": (
                    "buena" if resultado.get("calidad_imagen", 0) > 40 else "limitada"
                ),
                "modelo_usado": resultado.get("modelo_usado", "efficientnet"),
                "fuente": resultado.get("fuente", "desconocida"),
            }

            self.analisis_ia = analisis
            self.save(update_fields=["analisis_ia"])
            return analisis
            ...
            fallback = {
                ...
                "calidad_tecnica": "error",
                "error": str(e),
            }
            self.analisis_ia = fallback
            self.save(update_fields=["analisis_ia"])
            return fallback

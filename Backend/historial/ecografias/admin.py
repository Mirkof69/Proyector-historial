        ...
        if self.instance and self.instance.pk:
            if self.instance.placenta_localizacion:
                for value, label in self.LOCALIZACION_CHOICES:
                    if value and label.lower() == self.instance.placenta_localizacion.lower():
            ...
            if self.instance.placenta_inserccion_cordon:
                for value, label in self.INSERCION_CHOICES:
                    if value and label.lower() == self.instance.placenta_inserccion_cordon.lower():
...
class BiometriaFetalInline(admin.StackedInline):
    ...
    fieldsets = (
        ...
        ('Peso y Percentiles', {
            'fields': (
                ('peso_fetal_estimado', 'percentil_peso'),
            )
        }),
...
class AnatomiaFetalInline(admin.StackedInline):
    ...
    fieldsets = (
        ...
        ('Cara y Cuello', {
            'fields': (('perfil_facial_normal', 'labios_normales'),)
        }),
        ('Tórax', {
            'fields': (('corazon_normal', 'pulmones_normales'),)
        }),
        ('Abdomen', {
            'fields': (('estomago_normal', 'rinones_normales', 'vejiga_normal'),)
        }),
        ('Genitales', {
            'fields': (('genitales_visibles', 'sexo_fetal'),)
        }),
        ('Columna y Extremidades', {
            'fields': (
                'columna_normal',
                ('extremidades_superiores_normales', 'extremidades_inferiores_normales'),
            )
        }),
        ('Hallazgos Anormales', {
...
class AnexosFetalesInline(admin.StackedInline):
    ...
    fieldsets = (
        ...
        ('Cordón Umbilical', {
            'fields': (
                ('numero_vasos_cordon', 'circular_cordon'),
            ),
...
class ImagenEcografiaInline(admin.TabularInline):
    ...
    extra = 1
    fields = ('imagen', 'tipo_imagen', 'titulo', 'calidad_imagen', 'es_imagen_principal', 'orden')
...
class EcografiaAdmin(admin.ModelAdmin):
    ...
    def alertas_medicas_badge(self, obj):
        ...
        if hasattr(obj, 'anatomia') and obj.anatomia:
            if obj.anatomia.translucencia_nucal and obj.anatomia.translucencia_nucal > 3.5:
                alertas += 1
        ...
        if hasattr(obj, 'biometria') and obj.biometria:
            if obj.biometria.percentil_peso and (obj.biometria.percentil_peso < 10 or obj.biometria.percentil_peso > 90):
                alertas += 1
    ...
    def acciones_rapidas(self, obj):
        ...
        url_paciente = reverse('admin:pacientes_paciente_change', args=[obj.paciente.id])
    ...
    get_evaluacion_fcf.short_description = 'Evaluación FCF'
    ...
    def get_resumen_medico_completo(self, obj):
        ...
        resumen.append(f"• Vitalidad: {'Presente' if obj.vitalidad_fetal else 'Ausente'}")
        resumen.append(f"• Número de fetos: {obj.numero_fetos}")
        if obj.frecuencia_cardiaca_fetal:
            resumen.append(f"• FCF: {obj.frecuencia_cardiaca_fetal} lpm - {obj.get_evaluacion_fcf()}")
        resumen.append("")
        ...
        if hasattr(obj, 'anexos') and obj.anexos:
            ...
            if anexos.placenta_inserccion_cordon:
                resumen.append(f"• Inserción cordón: {anexos.placenta_inserccion_cordon}")
        ...
        if hasattr(obj, 'anatomia') and obj.anatomia:
            ...
            if ana.translucencia_nucal:
                if ana.translucencia_nucal > 3.5:
                    alertas.append("ALTO RIESGO CROMOSÓMICO: Translucencia nucal > 3.5mm")
    ...
    def get_recomendaciones_seguimiento(self, obj):
        ...
        if 6 <= obj.edad_gestacional_semanas <= 14:
            ...
            recomendaciones.append("Screening bioquímico de cromosomopatías si no realizado")
    ...
    def marcar_seguimiento_requerido(self, request, queryset):
        ...
        self.message_user(request, f'{updated} ecografías marcadas como que requieren seguimiento especial.')
    ...
    def marcar_seguimiento_completado(self, request, queryset):
        ...
        self.message_user(request, f'{updated} ecografías marcadas como seguimiento completado.')
    ...
    def generar_reporte_medico(self, request, queryset):
        ...
        self.message_user(request, f'Reportes médicos generados para {count} ecografías.')
    generar_reporte_medico.short_description = 'Generar reporte médico PDF'
    
    def duplicar_ecografia_control(self, request, queryset):
        if queryset.count() > 1:
            self.message_user(request, 'Solo puede duplicar una ecografía a la vez.', level=messages.ERROR)
            return
        
        ecografia = queryset.first()
        self.message_user(request, f'Ecografía de control programada para {ecografia.paciente.nombre_completo}.')
    ...
    def exportar_pdf_completo(self, request, queryset):
        count = queryset.count()
        self.message_user(request, f'Exportación PDF completa iniciada para {count} ecografías.')
...
class ImagenEcografiaAdmin(admin.ModelAdmin):
    ...
    def calidad_imagen_badge(self, obj):
        ...
        return format_html(
            '<span style="color: {}; font-weight: bold;">{}</span>',
            color, obj.get_calidad_imagen_display()
        )
    ...
    def marcar_como_principal(self, request, queryset):
        if queryset.count() > 1:
            self.message_user(request, 'Solo puede marcar una imagen como principal a la vez.', level='error')
            return
        
        imagen = queryset.first()
        if imagen:
            ImagenEcografia.objects.filter(
                ecografia=imagen.ecografia,
            ...
            self.message_user(request, f'Imagen "{imagen.titulo}" marcada como principal.')
    ...
    def analizar_con_ia(self, request, queryset):
        ...
        self.message_user(request, f'{procesadas} imágenes enviadas para análisis de IA.')

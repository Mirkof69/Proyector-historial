...
from .serializers import (
    ...
    CrecimientoFetalSerializer,
    DosisMedicamentosSerializer,
    HemorragiaObstetricaSerializer,
    RiesgoCromosomicoSerializer,
    RiesgoPreeclampsiaResumenSerializer,
    RiesgoPreeclampsiaSerializer,
    ScoreBishopResumenSerializer,
    ScoreBishopSerializer,
    SufrimientoFetalSerializer,
)
...
class ScoreBishopViewSet(viewsets.ModelViewSet):
    ...
    def estadisticas(self, _request):
        ...
        evaluaciones_recientes = queryset.filter(
            fecha_evaluacion__gte=hace_una_semana,
        ).count()
        ...
        return Response(
            {
                ...
                "porcentajes": {
                    "inmaduro_pct": round((inmaduro / total_evaluaciones * 100), 1)
                    if total_evaluaciones > 0
                    else 0,
                    "moderado_pct": round((moderado / total_evaluaciones * 100), 1)
                    if total_evaluaciones > 0
                    else 0,
                    "maduro_pct": round((maduro / total_evaluaciones * 100), 1)
                    if total_evaluaciones > 0
                    else 0,
                },
            },
...
class RiesgoPreeclampsiaViewSet(viewsets.ModelViewSet):
    ...
    def estadisticas(self, _request):
        ...
        total_evaluaciones = queryset.count()
        riesgo_promedio = queryset.aggregate(Avg("riesgo_porcentaje"))[
            "riesgo_porcentaje__avg"
        ]

        # Clasificación por riesgo
        alto_riesgo = queryset.filter(clasificacion_riesgo="Alto Riesgo").count()
        riesgo_intermedio = queryset.filter(
            clasificacion_riesgo="Riesgo Intermedio",
        ).count()
        ...
        factores = {
            ...
            "diabetes": queryset.filter(
                Q(diabetes_tipo1=True) | Q(diabetes_tipo2=True),
            ).count(),
            "antecedente_preeclampsia": queryset.filter(
                antecedente_preeclampsia=True,
            ).count(),
        ...
        return Response(
            {
                ...
                "porcentajes": {
                    "alto_riesgo_pct": round(
                        (alto_riesgo / total_evaluaciones * 100), 1,
                    )
                    if total_evaluaciones > 0
                    else 0,
                },
    ...
    def alto_riesgo(self, _request):
        ...
        return Response(
            {"total_alto_riesgo": alto_riesgo.count(), "pacientes": serializer.data},
        )
...
class CrecimientoFetalViewSet(viewsets.ModelViewSet):
    ...
    def estadisticas(self, _request):
        ...
        total_evaluaciones = queryset.count()
        peso_promedio = queryset.aggregate(Avg("peso_fetal_estimado"))[
            "peso_fetal_estimado__avg"
        ]

        # Clasificaciones
        peg = queryset.filter(restriccion_crecimiento=True).count()  # Pequeño para EG
        aeg = queryset.filter(
            restriccion_crecimiento=False, macrosomia=False,
        ).count()  # Adecuado
        ...
        return Response(
            {
                ...
                "porcentajes": {
                    "restriccion_crecimiento_pct": round(
                        (peg / total_evaluaciones * 100), 1,
                    )
                    if total_evaluaciones > 0
                    else 0,
                    "macrosomia_pct": round((geg / total_evaluaciones * 100), 1)
                    if total_evaluaciones > 0
                    else 0,
                },
            },
    ...
    def seguimiento_crecimiento(self, _request, _pk=None):
        ...
        evaluaciones_previas = CrecimientoFetal.objects.filter(
            paciente_id=evaluacion.paciente_id, embarazo_id=evaluacion.embarazo_id,
        ).order_by("edad_gestacional_semanas")

        # Crear curva de crecimiento
        curva_peso = []
        for eval_previa in evaluaciones_previas:
            if eval_previa.peso_fetal_estimado:
                curva_peso.append(
                    {
                        "edad_gestacional": eval_previa.edad_gestacional_semanas
                        + (eval_previa.edad_gestacional_dias or 0) / 7,
                        "peso": eval_previa.peso_fetal_estimado,
                        "percentil": eval_previa.percentil_peso,
                        "fecha": eval_previa.fecha_evaluacion,
                    },
                )
...
class RiesgoCromosomicoViewSet(viewsets.ModelViewSet):
    ...
    def estadisticas(self, _request):
        ...
        tn_promedio = queryset.aggregate(Avg("translucencia_nucal"))[
            "translucencia_nucal__avg"
        ]
        ...
        return Response(
            {
                ...
                "porcentajes": {
                    ...
                    "tn_elevada_pct": round((tn_elevada / total_evaluaciones * 100), 1)
                    if total_evaluaciones > 0
                    else 0,
                },
            },
...
class DosisMedicamentosViewSet(viewsets.ModelViewSet):
    ...
    def por_medicamento(self, request):
        ...
        indicaciones = (
            calculos.values("indicacion").annotate(count=Count("id")).order_by("-count")
        )
...
class HemorragiaObstetricaViewSet(viewsets.ModelViewSet):
    ...
    def estadisticas(self, _request):
        ...
        perdida_promedio = queryset.aggregate(Avg("perdida_sanguinea_estimada"))[
        ...
        # Causas principales
        causas = (
            queryset.values("causa_principal")
        ...
        return Response(
            {
                "resumen": {
                    ...
                    "perdida_promedio": round(perdida_promedio, 0)
                    if perdida_promedio
                    else 0,
                },
                "severidad": {
                ...
                "porcentajes": {
                    "control_pct": round((controladas / total_hemorragias * 100), 1)
                    if total_hemorragias > 0
                    else 0,
                    "cirugia_pct": round(
                        (requirieron_cirugia / total_hemorragias * 100), 1,
                    )
                    if total_hemorragias > 0
                    else 0,
                },
    ...
    def casos_severos(self, _request):
        ...
        casos_severos = (
            self.get_queryset()
            .filter(severidad_hemorragia__in=["Severa", "Masiva"])
        ...
        return Response(
            {"total_casos_severos": casos_severos.count(), "casos": serializer.data},
        )
...
class SufrimientoFetalViewSet(viewsets.ModelViewSet):
    ...
    def estadisticas(self, _request):
        ...
        riesgo_intermedio = queryset.filter(
            riesgo_sufrimiento_fetal="intermedio",
        ).count()
        ...
        return Response(
            {
                ...
                "porcentajes": {
                    "categoria_3_pct": round(
                        (categoria_3 / total_evaluaciones * 100), 1,
                    )
                    if total_evaluaciones > 0
                    else 0,
                    "intervencion_pct": round(
                        (intervenciones / total_evaluaciones * 100), 1,
                    )
                    if total_evaluaciones > 0
                    else 0,
                },
    ...
    @action(detail=False, methods=["get"])
    def alertas_activas(self, _request):
        """Casos que requieren intervención inmediata"""
        alertas = (
            self.get_queryset()
            .filter(requiere_intervencion_inmediata=True)
            .order_by("-fecha_evaluacion")
        )
        ...
        return Response(
            {"total_alertas": alertas.count(), "casos_urgentes": serializer.data},
        )
    ...
    def por_tipo_monitoreo(self, request):
        ...
        if not tipo:
            return Response(
                {"error": "tipo es requerido"}, status=status.HTTP_400_BAD_REQUEST,
            )

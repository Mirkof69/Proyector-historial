...
class DisponibilidadViewSet(viewsets.ModelViewSet):
    ...
    queryset = Disponibilidad.objects.select_related('medico').all().order_by('dia_semana', 'hora_inicio')
    ...
    def horarios_disponibles(self, request):
        ...
        # Verificar vigencia
        if disponibilidad.fecha_inicio_vigencia and fecha < disponibilidad.fecha_inicio_vigencia:
            return Response({
                'fecha': fecha,
                'dia_semana': fecha.strftime('%A'),
                'tiene_disponibilidad': False,
        ...
        if disponibilidad.fecha_fin_vigencia and fecha > disponibilidad.fecha_fin_vigencia:
            return Response({
                'fecha': fecha,
                'dia_semana': fecha.strftime('%A'),
                'tiene_disponibilidad': False,
        ...
        ).values_list('hora_cita', 'id', 'paciente__nombre', 'paciente__apellido_paterno')
...
class CitaViewSet(viewsets.ModelViewSet):
    ...
    queryset = Cita.objects.select_related(
        ...
        'confirmada_por',
        'creado_por'
    ).prefetch_related('historial').all().order_by('-fecha_cita', '-hora_cita')
    ...
    def create(self, request, *args, **kwargs):
        """Crear nueva cita"""
        # Agregar usuario actual como creado_por
    ...
    def por_paciente(self, request):
        ...
        citas = self.queryset.filter(paciente_id=paciente_id).order_by('-fecha_cita', '-hora_cita')
    ...
    def no_asistio(self, request, pk=None):
        ...
        serializer = CitaDetailSerializer(cita)
        return Response({
            'mensaje': 'Marcado como no asistió',
            'cita': serializer.data
        })